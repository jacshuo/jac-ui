...
const ICON_MAP: Record<string, React.ComponentType<{ className?: string }>> = {
  ...
  // Audio
  '.mp3': Music, '.wav': Music, '.flac': Music, '.ogg': Music, '.aac': Music, '.m4a': Music,
  // Video
  '.mp4': Video, '.mkv': Video, '.avi': Video, '.mov': Video, '.wmv': Video, '.webm': Video,
  // Code
  '.ts': FileCode, '.tsx': FileCode, '.js': FileCode, '.jsx': FileCode, '.json': FileCode,
  '.html': FileCode, '.css': FileCode, '.scss': FileCode, '.py': FileCode, '.rs': FileCode,
  '.go': FileCode, '.java': FileCode, '.c': FileCode, '.cpp': FileCode, '.h': FileCode,
  '.sh': TerminalIcon, '.bat': TerminalIcon, '.ps1': TerminalIcon,
  // Documents
  '.md': FileText, '.txt': FileText, '.pdf': FileText, '.doc': FileText, '.docx': FileText, '.rtf': FileText,
  // Data
...
export function FileExplorer({
  ...
  useEffect(() => {
    ...
    const items = Array.from(selected).map((i) => filteredFiles[i]).filter(Boolean);
  ...
  const toggleDock = useCallback(() => {
    if (docked) {
      ...
      if (maximized) {
        setMaximized(false);
        if (preMaxState) { setPos(preMaxState.pos); setSize(preMaxState.size); }
      }
      setDocked(true);
      onMinimize?.();
    }
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (!windowRef.current?.contains(document.activeElement) && document.activeElement !== windowRef.current) return;
      if (e.key === 'Escape') {
    ...
    return (
      <div
        className={cn(
          ...
          dockSide === 'right' ? 'right-0 top-1/2 -translate-y-1/2' : 'left-0 top-1/2 -translate-y-1/2',
        )}
        style={cssVars}
      ...
      >
        ...
        >
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest text-(--fe-text)">{title}</span>
        </div>
      </div>
    );
  }
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="ml-2 hidden h-px w-20 md:block" style={{ background: `linear-gradient(90deg, var(--fe-accent), transparent)`, opacity: 0.3 }} />
        </div>
        ...
        <div className="flex items-center gap-0.5">
          ...
          {/* Maximize */}
          <button className="fe-btn" onClick={toggleMaximize} title={maximized ? 'Restore' : 'Maximize'}>
            {maximized ? <Minimize2 className="h-3.5 w-3.5" /> : <Maximize2 className="h-3.5 w-3.5" />}
          </button>
          {/* Close */}
          <button
            className="fe-btn hover:bg-red-500/40! hover:text-red-300!"
            onClick={() => { onClose?.(); setInternalVisible(false); }}
            title="Close"
          >
      ...
      {/* ── Toolbar / address bar ──────── */}
      <div
        className="fe-toolbar flex h-9 shrink-0 items-center gap-2 px-3"
      >
        ...
        <button
          ...
          style={inputMode === 'navigate' ? { color: 'var(--fe-accent)', background: 'color-mix(in srgb, var(--fe-accent) 15%, transparent)' } : undefined}
          onClick={() => setInputMode((m) => (m === 'search' ? 'navigate' : 'search'))}
          title={inputMode === 'search' ? 'Switch to path navigation' : 'Switch to file search'}
        >
          {inputMode === 'search' ? <Search className="h-3 w-3" /> : <CornerRightDown className="h-3 w-3" />}
        </button>
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter' && searchQuery.trim()) {
                if (inputMode === 'navigate') {
                  onPathSubmit?.(searchQuery.trim()) ?? onNavigate?.(searchQuery.trim());
                }
      ...
      <div className="flex flex-1 min-h-0">
        {/* File list/grid */}
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-1.5" onClick={(e) => {
          if (e.target === e.currentTarget) { setSelected(new Set()); setInspectedIndex(null); }
        }}>
            ...
            <div className="space-y-px">
              {filteredFiles.map((item, i) => {
                ...
                return (
                  ...
                  >
                    <Icon className={cn('h-4 w-4 shrink-0', item.type === 'directory' ? 'text-(--fe-accent)' : 'text-(--fe-text-muted)')} />
                    <span className="flex-1 truncate text-xs font-medium">{item.name}</span>
                    {ext && item.type === 'file' && (
                      <span className="shrink-0 rounded bg-(--fe-badge-bg) px-1.5 py-0.5 font-mono text-[9px] uppercase text-(--fe-text-muted)">{ext}</span>
                    )}
                    {item.size != null && (
                      <span className="shrink-0 font-mono text-[10px] text-(--fe-text-muted)">{formatSize(item.size)}</span>
                    )}
                    {item.modifiedAt && (
                      <span className="hidden shrink-0 font-mono text-[10px] text-(--fe-text-muted) lg:inline">{formatDate(item.modifiedAt)}</span>
                    )}
                  </div>
                );
              })}
              ...
              {filteredFiles.map((item, i) => {
                ...
                return (
                  <div
                    ...
                    onDoubleClick={() => handleDoubleClick(i)}
                  >
                    <Icon className={cn('h-7 w-7', item.type === 'directory' ? 'text-(--fe-accent)' : 'text-(--fe-text-muted)')} />
                    <span className="w-full truncate text-[10px] font-medium leading-tight">{item.name}</span>
                    {ext && item.type === 'file' && (
                      <span className="font-mono text-[8px] uppercase text-(--fe-text-muted)">{ext}</span>
                    )}
                  </div>
                );
              })}
        ...
        >
          {inspectedFile && (
            <div
              className="fe-panel flex h-full w-56 flex-col border-l border-(--fe-border) p-4"
            >
              {/* File icon large */}
              <div className="mb-3 flex flex-col items-center">
                {React.createElement(getFileIcon(inspectedFile), {
                  className: cn('h-10 w-10 mb-2', inspectedFile.type === 'directory' ? 'text-(--fe-accent)' : 'text-(--fe-text-muted)'),
                })}
              ...
              <div className="space-y-2.5 text-[10px]">
                <InfoRow label="Type" value={inspectedFile.type === 'directory' ? 'Directory' : (inspectedFile.mimeType || (inspectedFile.extension || getExtension(inspectedFile.name)) || 'File')} />
                {inspectedFile.size != null && <InfoRow label="Size" value={formatSize(inspectedFile.size)} />}
                <InfoRow label="Path" value={inspectedFile.path} mono />
                {inspectedFile.extension && <InfoRow label="Extension" value={inspectedFile.extension} />}
                {inspectedFile.modifiedAt && <InfoRow label="Modified" value={formatDate(inspectedFile.modifiedAt)} />}
                {inspectedFile.createdAt && <InfoRow label="Created" value={formatDate(inspectedFile.createdAt)} />}
              </div>
              ...
              <div className="mt-auto flex flex-col gap-1.5">
                {inspectedFile.type === 'file' && onFileOpen && (
                  <button
                    ...
                    style={{ background: `color-mix(in srgb, var(--fe-accent) 20%, transparent)`, border: `1px solid color-mix(in srgb, var(--fe-accent) 30%, transparent)` }}
      ...
      {/* ── Status bar ─────────────────── */}
      <div
        className="fe-statusbar flex h-6 shrink-0 items-center justify-between px-3 text-[9px] font-mono text-(--fe-text-muted)"
      >
      ...
      {resizable && !maximized && (
        <>
          {/* edges */}
          <div className="absolute top-0 left-2 right-2 h-1.5 cursor-ns-resize" onMouseDown={(e) => handleResizeStart('n', e)} />
          <div className="absolute bottom-0 left-2 right-2 h-1.5 cursor-ns-resize" onMouseDown={(e) => handleResizeStart('s', e)} />
          <div className="absolute top-2 left-0 bottom-2 w-1.5 cursor-ew-resize" onMouseDown={(e) => handleResizeStart('w', e)} />
          <div className="absolute top-2 right-0 bottom-2 w-1.5 cursor-ew-resize" onMouseDown={(e) => handleResizeStart('e', e)} />
          {/* corners */}
          <div className="absolute top-0 left-0 h-3 w-3 cursor-nwse-resize" onMouseDown={(e) => handleResizeStart('nw', e)} />
          <div className="absolute top-0 right-0 h-3 w-3 cursor-nesw-resize" onMouseDown={(e) => handleResizeStart('ne', e)} />
          <div className="absolute bottom-0 left-0 h-3 w-3 cursor-nesw-resize" onMouseDown={(e) => handleResizeStart('sw', e)} />
          <div className="absolute bottom-0 right-0 h-3 w-3 cursor-nwse-resize" onMouseDown={(e) => handleResizeStart('se', e)} />
        </>
      )}
      ...
      <Dialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen}>
        <DialogContent size="sm">
          ...
          <DialogFooter>
            <Button intent="ghost" onClick={() => setDeleteConfirmOpen(false)}>Cancel</Button>
            <Button
              intent="danger"
              onClick={() => {
                const items = Array.from(selected).map((i) => filteredFiles[i]).filter(Boolean);
...
function InfoRow({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div>
      <div className="mb-0.5 font-semibold uppercase tracking-wider text-(--fe-text-muted)">{label}</div>
      <div className={cn('text-(--fe-text) break-all leading-snug', mono && 'font-mono text-[9px]')}>
        {value}
      </div>
    </div>
  );
